...
function MetaxsfrPage() {
  const [isCopied, setIsCopied] = useState(false)
  const [isIframeLoading, setIsIframeLoading] = useState(true)
  
  const metaxsfr_file = "/scifr/metaxsfr.example.html"
  const metaxsfr_commands = ["metaxsfr --reports 'bracken/*.txt' --report_type bracken --report_db GTDB --min_percent_abundance 0.01"]
  ...
  const handleDownload = () => {
    ...
    link.href = metaxsfr_file
    link.download = metaxsfr_file.split('/').pop() || ''
  ...
  const handleOpenInNewTab = () => {
    window.open(metaxsfr_file, '_blank')
  }
        ...
        <div className="px-4 flex flex-col space-y-4 mx-auto w-full max-w-6xl">
          <div>
            <h1 className="text-4xl font-bold text-left">
              Metagenome Taxonomic Explorer in a <span className='bg-clip-text text-transparent bg-gradient-to-r from-purple-400 via-pink-500 to-red-500'>Single-File Report.</span>
            </h1>
            <p className="text-xl text-base-content/70">
              Compile and explore results of Kraken, Bracken, and Metaphlan4 reports. 
              <a href="https://github.com/nalarbp/metaxsfr" target='_blank' className="text-blue-500 hover:underline" rel="noreferrer"> Learn more</a>
            </p>
          </div>
        
          <div className="mockup-code overflow-scroll bg-gray-800 text-green-400 p-4 rounded-lg">
            {metaxsfr_commands.map((command, index) => (
          ...
          <div className="w-full bg-base-200 border rounded-lg">
            <div className="p-4 border-b bg-base-100 rounded-t-lg">
              <div className="flex w-full flex-row items-center justify-between flex-wrap gap-2">
                <h3 className="text-lg font-semibold">METAXSFR Sample Report</h3>
              ...
              {isIframeLoading && (
                <div className="absolute inset-0 flex items-center justify-center bg-base-100 z-10">
                  <div className="flex flex-col items-center gap-4">
                    ...
                    <p className="text-base-content/70">Loading METAXSFR tool...</p>
                  </div>
                </div>
              )}
              <iframe
