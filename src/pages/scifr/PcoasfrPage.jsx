...
function PcoasfrPage() {
  const [isCopied, setIsCopied] = useState(false)
  const [isIframeLoading, setIsIframeLoading] = useState(true)
  
  const pcoasfr_file = "/scifr/pcoasfr.example.html"
  const pcoasfr_commands = ['pcoa_sfir.py -d similarityMatrix.tab -t pcoa_sfir_template.html -o sfir_out.html']
  ...
  const handleDownload = () => {
    ...
    link.href = pcoasfr_file
    link.download = pcoasfr_file.split('/').pop() || ''
  ...
  const handleOpenInNewTab = () => {
    window.open(pcoasfr_file, '_blank')
  }
        ...
        <div className="px-4 flex flex-col space-y-4 mx-auto w-full max-w-6xl">
          <div>
            <h1 className="text-4xl font-bold text-left">
              PCoA interactive chart in a <span className='bg-clip-text text-transparent bg-gradient-to-r from-purple-400 via-pink-500 to-red-500'>Single-File Report.</span>
            </h1>
            <p className="text-xl text-base-content/70">
              Simple example on running PCoA and displaying the results in a single-file interactive scatter chart.
            </p>
            <p className="text-md text-base-content/60">
              Download a Python script that generates a single-file report with an interactive PCoA chart
              <a href="#" className="text-blue-500"> here</a>.
            </p>
          </div>
        
          <div className="mockup-code overflow-scroll bg-gray-800 text-green-400 p-4 rounded-lg">
            {pcoasfr_commands.map((command, index) => (
          ...
          <div className="w-full bg-base-200 border rounded-lg">
            <div className="p-4 border-b bg-base-100 rounded-t-lg">
              <div className="flex w-full flex-row items-center justify-between flex-wrap gap-2">
                <h3 className="text-lg font-semibold">PCoA-SFR Sample Report</h3>
              ...
              {isIframeLoading && (
                <div className="absolute inset-0 flex items-center justify-center bg-base-100 z-10">
                  <div className="flex flex-col items-center gap-4">
                    ...
                    <p className="text-base-content/70">Loading PCoA-SFR tool...</p>
                  </div>
                </div>
              )}
              <iframe
